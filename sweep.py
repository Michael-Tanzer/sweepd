...
def migrate_sweep(sweep_id):
    """
    Migrates legacy ~/.sweeps/<id>.txt and <id>_ran.txt to new layout.
    Writes configs/<id>.meta.toml, configs/<id>.runs.txt, ran/<id>.txt.
    Idempotent if new layout already exists (skips).
    """
    if os.path.exists(_meta_path(sweep_id)):
        return False
    legacy = _legacy_sweep_path(sweep_id)
    if not os.path.exists(legacy):
        raise FileNotFoundError(f"Legacy sweep file not found: {legacy}")
    if tomli_w is None:
        raise RuntimeError("tomli-w is required for migration; install with: uv add tomli-w")
    command, param_lines = _load_legacy_sweep(sweep_id)
    os.makedirs(get_configs_dir(), exist_ok=True)
    os.makedirs(get_ran_dir(), exist_ok=True)
    with open(_meta_path(sweep_id), "wb") as f:
        tomli_w.dump({"command": command}, f)
    ...
    legacy_ran = _legacy_ran_path(sweep_id)
    if os.path.exists(legacy_ran):
        with open(legacy_ran, "r") as fr:
            with open(ran_path, "w") as fw:
                for line in fr:
                    line = line.strip()
                    if line:
                        fw.write(run_hash(line) + RAN_SEP + line + "\n")
    return True
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="File-based sweep manager (run subcommand or pass sweep_id for run)"
    )
    parser.add_argument("sweep_id", help="Sweep ID (configs/<id>.meta.toml, etc.)")
    args = parser.parse_args()
    sweep_run(args.sweep_id)
